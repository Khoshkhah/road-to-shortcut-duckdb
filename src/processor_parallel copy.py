...
def process_chunk_phase4(args):
    ...
    Uses Phase 3 logic with max_res filtering and deactivation.
    Returns: (cell_id, deactivated_df, count, timing_info)
    """
    ...
    try:
        ...
        # Load cell data with max_res column
        cell_table = f"cell_{cell_id}"
        con.execute(f"""
            CREATE TABLE {cell_table} AS 
            SELECT from_edge, to_edge, cost, via_edge, lca_res, 
                   inner_cell, outer_cell, inner_res, outer_res,
                   GREATEST(inner_res, outer_res) AS max_res
            FROM cell_df
        """)
        
        # Create deactivated table to collect all deactivated shortcuts
        con.execute("""
            CREATE TABLE deactivated (
                from_edge BIGINT, to_edge BIGINT, cost DOUBLE, via_edge BIGINT,
                lca_res INTEGER, inner_cell BIGINT, outer_cell BIGINT, 
        ...
        for target_res in range(partition_res + 1, 16):
            # 1. Filter shortcuts at this resolution (same logic as Phase 3)
            con.execute(f"""
                CREATE OR REPLACE TABLE working AS
                SELECT * FROM {cell_table} 
                WHERE max_res >= {target_res} AND lca_res <= {target_res}
            """)
            
            working_count = con.execute("SELECT count(*) FROM working").fetchone()[0]
            
            if working_count == 0:
                continue
            
            # 2. Split activable vs non_activable (based on max_res)
            # Activable: shortcuts NOT in working, to process at higher resolutions
            con.execute(f"""
                CREATE OR REPLACE TABLE activable AS
                SELECT * FROM {cell_table} 
                WHERE max_res >= {target_res} AND lca_res > {target_res}
            """)
            
            con.execute(f"""
                CREATE OR REPLACE TABLE non_activable AS
                SELECT * FROM {cell_table} 
                WHERE max_res < {target_res}
            """)
            
            # 3. Deactivate non_activable
            con.execute("""
                INSERT INTO deactivated
                SELECT from_edge, to_edge, cost, via_edge, lca_res, 
                       inner_cell, outer_cell, inner_res, outer_res
            ...
            # 4. Add current_cell to working for SP
            con.execute("ALTER TABLE working DROP COLUMN IF EXISTS current_cell")
            con.execute("ALTER TABLE working ADD COLUMN current_cell BIGINT")
            con.execute(f"""
                UPDATE working SET current_cell = h3_parent(inner_cell::BIGINT, {target_res}) 
                WHERE inner_res >= {target_res}
            """)
            con.execute(f"""
                UPDATE working SET current_cell = h3_parent(outer_cell::BIGINT, {target_res}) 
                WHERE current_cell IS NULL AND outer_res >= {target_res}
            """)
            
            # Determine method for this resolution
            if sp_method == "HYBRID":
                method = "PURE" if target_res >= hybrid_res else "SCIPY"
            else:
                method = sp_method
            
            # 5. Run SP on working
            res_start = time.time()
            _run_shortest_paths_worker(con, "working", method=method)
            timing_info.append((target_res, method, time.time() - res_start))
            
            # 6. Update cell table: UNION working + activable
            con.execute(f"DROP TABLE IF EXISTS {cell_table}")
            con.execute(f"""
                CREATE TABLE {cell_table} AS
                SELECT 
            ...
            con.execute("DROP TABLE IF EXISTS activable")
            con.execute("DROP TABLE IF EXISTS non_activable")
        
        # Get remaining in cell table + deactivated as final result
        con.execute(f"""
            INSERT INTO deactivated
            SELECT from_edge, to_edge, cost, via_edge, lca_res, 
                   inner_cell, outer_cell, inner_res, outer_res
        ...
        total_time = time.time() - start_time if 'start_time' in dir() else sum(t for _, _, t in timing_info)
        
        return (cell_id, result_df, initial_count, final_count, timing_info, total_time)
    
    except Exception as e:
        import traceback
        traceback.print_exc()
        return (cell_id, None, 0, 0, [], 0)
    finally:
        con.close()
...
class ParallelShortcutProcessor:
    ...
    def process_forward_phase1_parallel(self):
        ...
        if all_timing_info:
            ...
            if pure_times:
                logger.info(f"  PURE timing: {len(pure_times)} calls, total {format_time(sum(pure_times))}, avg {sum(pure_times)/len(pure_times):.2f}s")
            if scipy_times:
                logger.info(f"  SCIPY timing: {len(scipy_times)} calls, total {format_time(sum(scipy_times))}, avg {sum(scipy_times)/len(scipy_times):.2f}s")
    ...
    def process_backward_phase4_parallel(self):
        ...
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for i, future in enumerate(as_completed(futures), 1):
                cell_id, result_df, input_count, output_count, timing_info, cell_time = future.result()
                all_timing_info.extend(timing_info)
                ...
                if result_df is not None and len(result_df) > 0:
                    ...
                    self.con.execute(f"""
                        ...
                        FROM result_df
                    """)
                    total_deactivated += output_count
                
                # Drop cell table
                self.con.execute(f"DROP TABLE IF EXISTS cell_{cell_id}")
                logger.info(f"  [{i}/{len(cell_ids)}] Cell {cell_id}: {input_count:,} -> {output_count:,} ({format_time(cell_time)})")
